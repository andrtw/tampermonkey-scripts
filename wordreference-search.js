...
class BoxController {
    ...
    searchTerm(term, x, y) {
        ...
        fetch(`${BASE_URL}/${LANG}/${term}`)
            ...
            .then((html) => {
                ...
                const word = headerWordElem.childElementCount > 0 ? headerWordElem.childNodes[0].innerHTML : headerWordElem.innerHTML;
                const pron = this.#getPronunciation(dom.querySelector('#pronunciation_widget'));
                const audioUrl = this.#getAudioUrl(dom.querySelector('#listen_widget script')?.text);
    ...
    #populateBox(term, pron, audioUrl) {
        ...
        logoElem.onclick = (e) => { window.open(`${BASE_URL}/${LANG}/${term}`); };
    ...
    #getPronunciation(pronElem) {
        ...
        const deleteElems = pronElem.querySelectorAll('span[style="font-size:12px"]');
