...
function buildGoogleTranslate(term, sourceLang, targetLang) {
    ...
    elem.onclick = (e) => {
        window.open(`https://translate.google.com/?sl=${sourceLang}&tl=${targetLang}&text=${term}&op=translate`);
    };
    return elem;
}
...
function buildLangSwitcherItem(fromLang, toLang) {
    ...
    const rightArrow = buildLangSwitcherArrow('→', `${fromLang} to ${toLang}`, (e) => {
        const path = window.location.pathname.split('/')[2];
        const lang = `${fromLang}${toLang}`;
        window.location = `${origin}/${lang}/${path}`;
    });
    const leftArrow = buildLangSwitcherArrow('←', `${toLang} to ${fromLang}`, (e) => {
        const path = window.location.pathname.split('/')[2];
